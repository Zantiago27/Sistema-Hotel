...
public class ComprobanteService {
    ...
    public ComprobanteResponseDto emitir(EmitirComprobanteDto dto) {
        ...
        List<ReservaHabitacion> habitaciones = reservaHabitacionRepository.findAllByReservaId(reserva.getId());

        for (ReservaHabitacion rh : habitaciones) {
            Habitacion hab = habitacionRepository.findById(rh.getHabitacionId())
                    .orElse(null);
            String descripcion = "Habitación " +
                    (hab != null ? hab.getNumero() : "") +
                    " - " + rh.getTipoAlquiler();
            ...
            detalles.add(ComprobanteDetalle.builder()
                    ...
                    .subtotal(rh.getPrecioAplicado())
                    .build());

            subtotal = subtotal.add(rh.getPrecioAplicado());
        }
        ...
        for (Consumo c : consumos) {
            String nombreProducto = productoRepository.findById(c.getProductoId())
                    .map(Producto::getNombre).orElse("Producto");
        ...
        // Línea de adelanto como descuento
        BigDecimal adelanto = reserva.getAdelanto() != null
                ? reserva.getAdelanto() : BigDecimal.ZERO;

        if (adelanto.compareTo(BigDecimal.ZERO) > 0) {
            detalles.add(ComprobanteDetalle.builder()
                    .descripcion("Adelanto pagado")
                    .cantidad(1)
                    .precioUnit(adelanto.negate())
                    .subtotal(adelanto.negate())
                    .build());
        }

        // Total final = subtotal - adelanto
        BigDecimal total = subtotal.subtract(adelanto);
        if (total.compareTo(BigDecimal.ZERO) < 0) total = BigDecimal.ZERO;

        // Serie y número correlativo
        String serie = getSerie(dto.getTipo());
        Integer ultimoNumero = comprobanteRepository.findUltimoNumero(sucursalId, serie);
